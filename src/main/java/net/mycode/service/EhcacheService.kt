...
open class EhcacheService {
     ...
     *  jCacheCacheManager是spring自动发现声明的bean，idea在我的项目里找不到所以报错。这个声明bean在JCacheCacheConfiguration类里面
     * */
    @Autowired
    ...
    open fun saveUser(name: String): User {
        ...
        return User(100, " 我的名字叫：$name ${cache.name}", 33, Date())
    }

    // 手动自动开启，如果不存在缓存中直接保存然后返回，存在则立即从缓存返回
    open fun saveUser001(name: String): User {
        var cache = jCacheCacheManager.getCache("people")
        var u = User(100, " 我的名字叫：$name ${cache.name}", 39, Date())
